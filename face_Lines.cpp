...
vector<Rect> truths;	//circles
vector<Rect> detect;	//final detection
...
void iouVal(vector<Rect> faces){
	...
	if(truths.empty())
	{
		truths.push_back(Rect(strongest[0].x,strongest[0].y,strongest[0].width,strongest[0].width));
	}
...
Mat houghLines( Mat img )
{
  ...
  cout <<endl<< "Max intersections between lines : " << maxIn/50 << " at " << Point(maxi_Y,maxi_X) << endl;
    ...
    for(int x = 0 ; x < grad.cols ; x++)
    {
      ...
      if(vote > voteTh){  //Checking if vote is above the threshold
        ...
        circle(img_h, Point(x,y), 1, cvScalar(255,0,0), 2);        //Printing centre pt
				...
				int rht = radius*2;
				int rwt = radius*2;
				truths.push_back(Rect(rx,ry,rht,rwt)); //send all circle above threshold
